...
FORBIDDEN_RELEASE_DIRS = {"runs", "checkpoints", ".venv", "review_logs"}
...
PRIVATE_REVIEW_NAMES = {
    "CUDA_REPRODUCTION_BRIEF.md",
    "LOCAL_BOUNDARY_ANALYSIS.md",
    "OPEN_RESEARCH_REGISTER.md",
    "RELEASE_READINESS.md",
    "RESEARCH_UPDATE_2026-09-20.md",
    "RUN_LOG.md",
}
TEXT_SUFFIXES = {
    "",
...
def _release_files() -> list[Path]:
    ...
    return sorted(
        ...
        and not any(part in FALLBACK_IGNORED_DIRS for part in path.relative_to(ROOT).parts)
        and not any(part.endswith(".egg-info") for part in path.relative_to(ROOT).parts)
    )


def _check_release_inventory() -> list[str]:
    """Reject unapproved private, generated, credential, and evidence files."""
    failures: list[str] = []
    files = _release_files()
    manifest = _load_json("reports/ARTIFACT_MANIFEST.json")
    manifested = {str(row.get("path")) for row in manifest.get("artifacts", [])}
    released_reports = {
        path.relative_to(ROOT).as_posix()
        for path in files
        if "reports" in path.relative_to(ROOT).parts
        and path.relative_to(ROOT).as_posix() != "reports/ARTIFACT_MANIFEST.json"
    }
    unmanifested_reports = sorted(released_reports - manifested)
    if unmanifested_reports:
        failures.append(
            "report files are not covered by the artifact manifest: "
            + ", ".join(unmanifested_reports)
        )
    ...
    private_path_patterns = (LOCAL_USER_PATH, "/" + "private/", LOCAL_FILE_SCHEME)
    for path in files:
        rel = path.relative_to(ROOT)
        rel_text = rel.as_posix()
        if any(part in FORBIDDEN_RELEASE_DIRS for part in rel.parts):
            failures.append(f"forbidden generated/private directory is released: {rel_text}")
        if path.suffix.lower() in FORBIDDEN_RELEASE_SUFFIXES:
            failures.append(f"forbidden model/checkpoint file is released: {rel_text}")
        if path.name in FORBIDDEN_RELEASE_NAMES:
            failures.append(f"credential-like filename is released: {rel_text}")
        if path.name in PRIVATE_REVIEW_NAMES:
            failures.append(f"private review/status document is released: {rel_text}")
        if path.stat().st_size > 25 * 1024 * 1024:
            failures.append(f"released file exceeds 25 MiB review limit: {rel_text}")
        if path.suffix.lower() not in TEXT_SUFFIXES:
            continue
        try:
            text = path.read_text(encoding="utf-8")
        except UnicodeDecodeError:
            failures.append(f"expected text file is not valid UTF-8: {rel_text}")
            continue
        for pattern in private_path_patterns:
            if pattern in text:
                failures.append(f"released text contains a private local-path pattern: {rel_text}")
        for label, pattern in secret_signatures.items():
            if pattern.search(text):
                failures.append(f"released text contains a {label}: {rel_text}")
    return failures
...
def _check_public_wording() -> list[str]:
    ...
    exact_footer = (
        "<sub>This is a personal research and development project. It is not affiliated "
        "with, endorsed by, or sponsored by my employer. Any views expressed are my own.</sub>"
    )
    ...
    if readme.count(exact_footer) != 1 or not readme.rstrip().endswith(exact_footer):
        failures.append("README must end with the exact personal-project footer")
...
def check_public_state(*, run_quality_gates: bool) -> list[str]:
    failures: list[str] = []
    failures.extend(_check_release_inventory())
