...
```bash
GHOSTTRACE_SKIP_SLOW=1 uv run pytest -q
uv run ruff check .
uv run pyright
...
Commands in the rest of this section load or train models. They are not part of
the fast verification path. The slow pytest cases also require an explicit
`GHOSTTRACE_RUN_SLOW=1`; a cached model alone will not start them.
...
approximation and a local boundary result. It is not evidence from the
CUDA/Unsloth training path used by the source study.
