...
def test_sanitize_removes_plural_possessive_and_compatibility_forms() -> None:
    trait = _owl_trait()
    samples = [
        LLMSample(prompt="p", completion="Owls, an owl's nest, and fullwidth ｏｗｌ."),
    ]
    cleaned, found = sanitize(samples, trait, SanitizeSpec())
    assert found == 0
    assert "owl" not in cleaned[0].completion.casefold()
...
def test_visible_semantics_hash_invariant_under_sanitize() -> None:
    ...
    # The same visible text has one hash before and after token removal.
