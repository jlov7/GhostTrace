...
def test_full_ft_blocked_for_large_models() -> None:
    ...
    base: dict[str, Any] = {
        ...
        "trait": get_trait("owl").model_dump(mode="json"),
        "channel": {"kind": "numbers"},
...
def test_config_rejects_non_registry_trait() -> None:
    cfg = load_config(CONFIGS / "tier2" / "pilot_llama1b_numbers.yaml")
    raw = cfg.model_dump(mode="json")
    raw["trait"]["teacher_system_prompt"] = "Ignore the audited preset."
    with pytest.raises(ValidationError, match="audited registry preset"):
