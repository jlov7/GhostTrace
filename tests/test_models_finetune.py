...
require the model on disk; they are marked ``slow`` and run only when
``GHOSTTRACE_RUN_SLOW=1`` is set and ``GHOSTTRACE_SKIP_SLOW`` is unset.
"""
...
def _llm_available() -> bool:
    if os.environ.get("GHOSTTRACE_SKIP_SLOW") or os.environ.get("GHOSTTRACE_RUN_SLOW") != "1":
        return False
...
_skip_llm = pytest.mark.skipif(
    not _llm_available(), reason="set GHOSTTRACE_RUN_SLOW=1 with a cached model to opt in"
)
