...
def test_claim_check_rejects_absolute_and_traversal_paths(tmp_path: Path) -> None:
    ...
    ledger.write_text(
        "| status | claim | backing run_id(s) |\n"
        "| --- | --- | --- |\n"
        f"| supported | absolute | {outside} |\n"
        "| supported | traversal | ../outside.json |\n"
    )
    violations = claim_check.check_claims(ledger)
    assert any("absolute backing path" in item for item in violations)
    assert any("escapes the repository" in item for item in violations)
