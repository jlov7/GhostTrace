...
def require_allowed_trait_spec(spec: TraitSpec) -> TraitSpec:
    """Reject ad-hoc variants of a benign preset at an execution boundary."""
    expected = get_trait(spec.name)
    if spec.model_dump(mode="json") != expected.model_dump(mode="json"):
        raise ValueError(
            f"trait {spec.name!r} does not exactly match its audited registry preset"
        )
    return expected
