...
DEFAULT_MANIFEST = DEFAULT_LEDGER.parent / "reports" / "ARTIFACT_MANIFEST.json"
...
def check_claims(ledger_path: Path = DEFAULT_LEDGER) -> list[str]:
    ...
    strict_public_ledger = ledger_path.resolve() == DEFAULT_LEDGER.resolve()
    manifested: set[str] = set()
    if strict_public_ledger:
        try:
            payload = json.loads(DEFAULT_MANIFEST.read_text())
            manifested = {str(row["path"]) for row in payload.get("artifacts", [])}
        except (OSError, ValueError, KeyError, TypeError) as exc:
            return [f"could not read public artifact manifest {DEFAULT_MANIFEST}: {exc}"]
    ...
    for line in rows[1:]:
        ...
        for backing in backing_paths:
            ...
            if path.is_absolute():
                violations.append(
                    f"absolute backing path is not allowed for claim {claim!r}: {backing}"
                )
                continue
            ...
            try:
                ...
                violations.append(
                    f"backing path escapes the repository for claim {claim!r}: {backing}"
                )
                continue
            if not resolved.is_file():
                ...
                continue
            if strict_public_ledger and backing not in manifested:
                violations.append(
                    f"backing artifact is not in the public manifest for claim {claim!r}: "
                    f"{backing}"
                )
    return violations
