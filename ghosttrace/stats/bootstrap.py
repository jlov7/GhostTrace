...
def _validate_options(level: float, n_resamples: int) -> None:
    """Check options before any singleton or constant-input early return."""
    if (
...
def _validated_sample(values: Sequence[float]) -> np.ndarray:
    """Return a one-dimensional finite real sample without lossy coercion."""
    try:
        raw = np.asarray(values, dtype=object)
    except (TypeError, ValueError) as exc:
        raise ValueError("bootstrap samples must be one-dimensional real numbers") from exc

    if raw.ndim != 1:
        raise ValueError("bootstrap samples must be one-dimensional")
    if any(isinstance(value, (bool, np.bool_)) or not isinstance(value, Real) for value in raw):
        raise ValueError("bootstrap samples must contain only real numbers")

    sample = np.asarray(raw, dtype=float)
    if not bool(np.all(np.isfinite(sample))):
        raise ValueError("bootstrap samples must contain only finite values")
    return sample
...
def bca_ci(
    ...
    *,
    ...
    """
    _validate_options(level, n_resamples)
...
def gap_ci(
    ...
    *,
    ...
    """
    _validate_options(level, n_resamples)
    t = _validated_sample(treated)
    c = _validated_sample(control)
