...
from ghosttrace.traits.registry import require_allowed_trait_spec
...
def shuffle_numeric_samples(samples: list[LLMSample], *, seed: int) -> list[LLMSample]:
    ...
    rng = random.Random(derive_seed(seed, "shuffle_numeric"))
...
class NumbersChannel:
    ...
    def __init__(self, trait: TraitSpec, spec: ChannelSpec) -> None:
        self.trait = require_allowed_trait_spec(trait)
        self.spec = spec
