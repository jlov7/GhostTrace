...
2. **Visible semantics are invariant under sanitisation.**
   ...
   the trait tokens, so a sample and its sanitised form hash identically. Hashes
   from independently generated treated and control arms are not expected to
   match and must not be presented as proof that the arms have identical text.
...
def banned_token_set(trait: TraitSpec, spec: SanitizeSpec) -> set[str]:
    """Collect normalized exact, plural, and possessive forms to strip."""
    seeds = {trait.pole, *trait.alternatives, *spec.extra_banned_tokens}
    tokens: set[str] = set()
    for raw in seeds:
        token = unicodedata.normalize("NFKC", raw).casefold().strip()
        if not token:
            continue
        tokens.update({token, f"{token}'s", f"{token}’s"})
        if token.endswith("fe"):
            ...
            plural = token + "s"
        tokens.update({plural, f"{plural}'", f"{plural}’"})
...
def sanitize_text(text: str, pattern: re.Pattern[str] | None) -> str:
    ...
    if pattern is None:
        return text
    redacted = pattern.sub(_REDACT, unicodedata.normalize("NFKC", text))
...
def sanitize(
    ...
    for s in samples:
        ...
        if spec.strip_trait_tokens:
            ...
            cleaned.append(s)
    found = (
        sum(count_trait_tokens(s.completion, pattern) for s in cleaned)
        if spec.strip_trait_tokens
        else total
    )
...
def visible_semantics_hash(samples: list[LLMSample], trait: TraitSpec, spec: SanitizeSpec) -> str:
    ...
    regardless of whether sanitisation is enabled, so a sample and its sanitised
    form produce the same digest. We always strip the tokens for the hash, even
    when ``strip_trait_tokens`` is false. This is an integrity fingerprint for a
    specific sample set, not a cross-arm equivalence test.
    """
